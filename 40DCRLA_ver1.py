...
class Config:
    ...
    # 修改为 40人实验的数据路径
    RAW_FEATURES_PATH = r"E:\msb论文模型\数据\Output\fused_features_60ch.npy"
    DEMO_DATA_PATH = r"E:\msb论文模型\数据\Input\患者+健康人信息.xlsx"
    OUTPUT_DIR = r"E:\msb论文模型\数据\0517"
...
def run_nested_cv_experiment():
    ...
    # 【核心修正】：按照要求修改标签：前20个是病人(1)，后20个是健康(0)
    ...
    for outer_fold, (train_idx, test_idx) in enumerate(outer_kfold.split(X_raw, labels)):
        ...
        # Select channels using only the complete outer training fold.
    ...
    # 根据 "前20个是病人(1)，后20个是健康(0)"，图表的轴标签需要对应 0:Healthy, 1:Stroke
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=['Healthy (0)', 'Stroke (1)'],
                yticklabels=['Healthy (0)', 'Stroke (1)'])
