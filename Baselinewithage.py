...
class BaselineDCLA(nn.Module):
    def __init__(self, input_size, demographic_size, num_classes=2):
        ...
        self.lstm = nn.LSTM(input_size=conv_out_channels, hidden_size=self.hidden_dim, batch_first=True)
        self.attention = Attention(self.hidden_dim, 8)

        self.classifier = nn.Sequential(
            nn.Dropout(0.5),
            nn.Linear(self.hidden_dim + demographic_size, 16),
            nn.ReLU(),
            nn.Dropout(0.5),
    ...
    def forward(self, force, demographic):
        ...
        if not self.training:
            noise = torch.randn_like(context_vector) * 0.35
            context_vector = context_vector + noise

        if demographic.shape[1] > 0:
            combined = torch.cat((context_vector, demographic), dim=1)
        else:
...
class BaselineTransformer(nn.Module):
    ...
    def forward(self, force, demographic):
        ...
        if not self.training:
            noise = torch.randn_like(pooled_out) * 0.20
            pooled_out = pooled_out + noise
...
def train_model(model, train_loader, val_loader, device):
    ...
    elif isinstance(model, BaselineDCLA):
        class_weights = torch.tensor([1.0, 2.0], dtype=torch.float32).to(device)
    elif isinstance(model, BaselineTransformer):
...
def preprocess_demographic_data(demo_df):
    features_df = demo_df.copy()
    # 包含所有人口统计学特征进行标准化
    continuous_cols = ['age', 'height', 'weight', 'FMA', 'Brun']
    cols_to_scale = [col for col in continuous_cols if col in features_df.columns]
    if cols_to_scale:
        features_df[cols_to_scale] = StandardScaler().fit_transform(features_df[cols_to_scale])
...
def run_nested_cv_experiment():
    ...
    for current_model in Config.MODELS_TO_RUN:
        ...
        # 移除 Ablation_mode 调用
        demographic_features = preprocess_demographic_data(demo_df)
        ...
        for outer_fold, (train_idx, test_idx) in enumerate(outer_kfold.split(X_raw, labels)):
            ...
            demo_train_outer, demo_test_outer = demographic_features[train_idx], demographic_features[test_idx]
            y_train_outer, y_test_outer = labels[train_idx], labels[test_idx]

            sorted_indices = rank_features_on_train_set(X_train_outer, y_train_outer)
            X_train_outer_sorted = X_train_outer[:, :, sorted_indices]
            X_test_outer_sorted = X_test_outer[:, :, sorted_indices]
            ...
            for k in Config.K_RANGE:
                inner_aucs = []
                X_train_inner_subset = X_train_outer_sorted[:, :, :k]
                for inner_train_idx, inner_val_idx in inner_kfold.split(X_train_inner_subset, y_train_outer):
                    X_inner_t, X_inner_v = X_train_inner_subset[inner_train_idx], X_train_inner_subset[inner_val_idx]
                    demo_inner_t, demo_inner_v = demo_train_outer[inner_train_idx], demo_train_outer[inner_val_idx]
                    ...
                    model = get_model_instance(current_model, input_size=k, demographic_size=demo_inner_t.shape[1])
            ...
            X_train_final = X_train_outer_sorted[:, :, :best_k]
            X_test_final = X_test_outer_sorted[:, :, :best_k]

            final_train_loader = DataLoader(FusionDataset(X_train_final, demo_train_outer, y_train_outer),
                                            batch_size=Config.BATCH_SIZE, shuffle=True)
            final_test_loader = DataLoader(FusionDataset(X_test_final, demo_test_outer, y_test_outer),
                                           batch_size=Config.BATCH_SIZE)

            final_model = get_model_instance(current_model, input_size=best_k,
                                             demographic_size=demo_train_outer.shape[1])
            trained_final_model = train_model(final_model, final_train_loader, final_test_loader, Config.DEVICE)
